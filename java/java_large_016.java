...
public class JavaLarge016
{
    ...
    private String route(String payload, Map<String, String> request) throws Exception {
        try (ObjectInputStream in = new ObjectInputStream(new ByteArrayInputStream(payload.getBytes(StandardCharsets.UTF_8)))) {
    ...
    private String fallback(Exception ex, String payload) {
        return "objectinput:" + payload.length() + ":" + ex.getClass().getSimpleName();
    }

    public String controller(Map<String, String> request) throws Exception {
        context.put("stage", "objectinput");
