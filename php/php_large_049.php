...
final class PhpLarge049
{
    ...
    {
        $wrapper = unserialize($payload, ['allowed_classes' => [ToStringEnvelope::class]]);
        return (string) $wrapper;
    }

    private function handleFailure(Throwable $e, string $payload): string
    {
        return 'string-fallback:' . hash('crc32b', $payload);
    }

    public function middleware(array $request): string
    {
        $this->context['stage'] = 'toString';
        return $this->handle($request);
    }
}

final class ToStringEnvelope
{
    public string $inner = '';
    public function __toString(): string
    {
        return $this->inner === '' ? 'empty' : $this->inner;
    }
}
